  ...
  describe('/support', () => {
    ...
    it('Koa Server with Invalid Config:', done => {
      ...
      app({})
        ...
        .catch(e => {
          chai.should().equal(e.name, 'InvalidConfigError', 'Error not Invalid Config Error!')
          done()
        })
    })
    ...
    it('Koa Server with Config into ./config directory:', done => {
      ...
      app()
        .then(server => {
          ...
          chai.assert.equal('unknown-http-server', server.info.name, 'server.info.name is invalid!')
          chai.assert.equal('test', server.info.version, 'server.info.version is invalid!')
    ...
    it('/health', done => {
      chai
        ...
        .end((err, res) => {
          ...
          chai.assert.equal('1.0.0', res.body.server.version, 'server.version is invalid!')
